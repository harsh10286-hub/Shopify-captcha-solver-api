...
def solve_recaptcha_v2(site_key, page_url="https://example.com", invisible=True, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            # Use DETECTED site_key, not hardcoded!
            anchor_url = f"https://www.google.com/recaptcha/api2/anchor?ar=1&k={site_key}&co=aHR0cHM6Ly9jaGVja291dC5zaG9waWZ5LmNvbTo0NDM.&hl=en&v=V6_85qpc2Xf2sbe3xTnRte7m&size=invisible&cb={int(time.time() * 1000)}"
            ...
            # Use DETECTED site_key in reload, not hardcoded!
            post = session.post(reload_url, data=f"v=UFwvoDBMjc8LiYc1DKXiAomK&reason=q&c={token}&k={site_key}&co=aHR0cHM6Ly9jaGVja291dC5zaG9waWZ5LmNvbTo0NDM.&hl=en&size=invisible&chr=%5B61%2C36%2C84%5D&vh={random.randint(7000000000, 8000000000)}&bg=", headers={
...
def admin_sitechecker_test():
    """Test site - only adds if cart add succeeds (like eternal-tattoo-supply)"""
    global sites_data
